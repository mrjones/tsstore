...
$(TEST_BINDIR)/tsstore_test: $(TEST_SRCDIR)/tsstore_test.cc $(OBJDIR)/libgtest.a $(OBJDIR)/tsstore.o | $(TEST_BINDIR)
	$(CC) $(CFLAGS) -isystem $(GTESTDIR)/include -pthread $(OBJDIR)/tsstore.o $< $(OBJDIR)/libgtest.a -o $@
...
$(OBJDIR)/gtest-all.o: | $(OBJDIR)
	$(CC) $(CFLAGS) -isystem $(GTESTDIR)/include -I$(GTESTDIR) -L$(GTESTDIR)/lib -pthread -c $(GTESTDIR)/src/gtest-all.cc -o $(OBJDIR)/gtest-all.o
...
$(BINDIR)/tsstore: $(OBJDIR)/tsstore.o $(OBJDIR)/tsstore_main.o | $(BINDIR)
	$(CC) $(CFLAGS) $(OBJDIR)/tsstore.o $(OBJDIR)/tsstore_main.o -o $(BINDIR)/tsstore
